...
//	T_UWORD *addrSecond = videoRamBase + CONSOLE_ROW_SIZE*uwLines/4;
	...
	addrLast = videoRamBase + (CONSOLE_ROWS-uwLines)*VIDEO_FONT_HEIGHT *CONSOLE_COLS*VIDEO_FONT_WIDTH;

	//
	#if 0
	while(addrFirst < addrLast)
	{
		*addrFirst = *(addrFirst+ uwLines*CONSOLE_COLS*VIDEO_FONT_WIDTH*VIDEO_FONT_HEIGHT);
		addrFirst++;
	}
	#endif
	asm volatile("1:\n\t"
				...
				"b 1b\n\t"
				"2:\n\t"				
				::"r"(addrFirst), "r"(addrLast), "r"(uwLines*CONSOLE_COLS*VIDEO_FONT_WIDTH*VIDEO_FONT_HEIGHT*VIDEO_PIXEL_SIZE));
	
	// 屏幕内容向前移动lines行后，屏幕最后lines行应该显示为空白，没有任何字符，因此用空格代替
	memsetl (addrLast, uwLines*CONSOLE_COLS*VIDEO_FONT_WIDTH*VIDEO_FONT_HEIGHT, CONSOLE_BG_COL);
