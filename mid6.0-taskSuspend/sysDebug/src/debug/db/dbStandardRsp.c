...
{
	/* 启动初始化任务 */
	/*该问题主要是自动激活irtp任务，所以修改的思路就是判断绑定的任务是不是irtp任务就可*/
	/*最后在c命令处判断taskBind值，如果为0，则激活taInitTaskId；如果为1，则不激活*/
	if ((taInitTaskId != 0) && (0 == taskBind))
	{
		/* 激活初始化任务 */
                   ...
                   UINT32 *outSize)
{
    ...
    /*FIXME liyj 由于在单步返回时，要读st0寄存器（X86 浮点），当返回'\0'时，GDB能够正常往下走*/
    if(TA_INVALID_PARAM == ret)
    {
    	outbuf[0] = '\0';
    	*outSize = 1;
    	return (TA_OK);
