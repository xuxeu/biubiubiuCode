...
{
	/* 启动初始化任务 */
    //printk("-----taDebugTaskActivate----0000----taInitTaskId:0x%x----\n",taInitTaskId);
	if (taInitTaskId != 0)
	{
		/* 激活初始化任务 */
		if ((debugInfo->ctxType == TA_CTX_TASK_RTP) || (debugInfo->ctxType == TA_CTX_SYSTEM_RTP))
		{
		    //printk("-----taDebugTaskActivate----1111----taInitTaskId:0x%x----\n",taInitTaskId);
			taTaskCont(taInitTaskId);
		}
		else
		{
		    //printk("-----taDebugTaskActivate----2222----taInitTaskId:0x%x----\n",taInitTaskId);
...
{
    /* 清除单步位 */
    //printk("-----RspContinue----0000--------\n");
    taDebugContinue(debugInfo->context);

    /* 激活初始化任务 */
    //printk("-----RspContinue----1111--------\n");
    taDebugTaskActivate(debugInfo);
    //printk("-----RspContinue----2222--------\n");
                   ...
                   UINT32 *outSize)
{
    ...
    /* 设置"QCthreadId"到输出缓冲 */
#ifdef CONFIG_TA_LP64
    *outSize = (UINT32)sprintf((INT8*)(outbuf), "QC%lx",threadId);
#else
