...
BOOL taDebugArchInit(void)
{
	...
	taInstallVectorHandle(GENERAL_DB_INT, taOsExceptionHandler);

	/* 安装软件断点异常 */
	taInstallVectorHandle(GENERAL_TRAP_INT, taOsExceptionHandler);
#else
...
#ifdef __ARM__
	_func_excBaseHook = &taOsExceptionHandler;
    taInstallVectorHandle(12, taOsExceptionHandler);
...
T_TA_ReturnCode taDebugWriteSingleRegister(T_DB_ExceptionContext *context, UINT32 registerNum, const UINT8 *inBuf)
{
    ...
    //printk("hello 111\n");
    /* 参数有效性判断 */
    if ((registerNum >= DB_ARM_REGISTER_SUM))
    {
...
{
    /* 清除PS寄存器的单步标志位 */
    //context->registers[CPSR] &= (~DB_ARCH_PS_TRAP_BIT);
    //context->registers[CPSR] &= (~DB_ARCH_PS_TRAP_BIT);
    //printk("-----taDebugContinue----context->registers[CPSR]:0x%x--------\n",context->registers[CPSR]);
