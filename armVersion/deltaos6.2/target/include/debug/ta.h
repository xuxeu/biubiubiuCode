...
/*
  *change by liy on 2016-11-19:在64模式下，TA使用高2G的逻辑地址时，网卡直接关联的缓冲地址
  *必须是物理地址。
  */
  
#define LOCAL_MEM_PHYS_ADRS_MASK_TA        0x000000007ffffffful

#define LOCAL_MEM_VIRT_TO_PHYS_TA(addr)    ((addr) & (LOCAL_MEM_PHYS_ADRS_MASK_TA))

#else

#define LOCAL_MEM_VIRT_TO_PHYS_TA(addr)    (addr) 
...
/* 字符串格式化 */
/*extern int sprintf(INT8 *buffer,const char *  fmt,...);*//*del by chenh 20141015*/
 ...
 *     无
 */
BOOL taOsExceptionHandler(UINT32 vecNum,ESF *pEsf, ulong_t *pRegSet,BOOL error,ulong_t *pExcInfo);
