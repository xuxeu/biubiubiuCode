...
void rseGetRtpModuleInfo(T_TA_Packet *packet,UINT32 *outSize)
{
	...
	*ptr = 0;
	...
	len = sprintf((INT8 *)ptr,"%s,%s,%lx",rtpName, rtpInfo.pathName,(ulong_t)((WIND_RTP *)rtpId)->binaryInfo.textStart);
#else
	len = sprintf((INT8 *)ptr,"%s,%s,%x",rtpName, rtpInfo.pathName,(ulong_t)((WIND_RTP *)rtpId)->binaryInfo.textStart);
#endif
	ptr += (len+1);
