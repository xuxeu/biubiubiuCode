...
//#define MIPSXX_cav_cn3xxx

/* defines */
#define CAVIUM_JUMP_MASK (0x33 << 26)

/* bits 26 to 31 of cavium jump instructions */
#define BBIT0	0x32
#define BBIT032 0x36
#define BBIT1	0x3A
#define BBIT132	0x3E
...
void usrBreakpointSet
	...
	*addr = value;
				...
				//刷新cache
	taCacheTextUpdata(addr, sizeof(value));
...
/*
	...
	switch (policy)
	    ...
	    case VXDBG_LOCAL_FLUSH :
            ...
            printk("usrBreakpointSet   addr = 0x%x\r\n",addr);
...
void SetBreakAtNextPc(T_DB_ExceptionContext* context)
{
    ...
    //VXDBG_ISR_NEXT_PC() = wdbDbgGetNpc (&RegSet);
     VXDBG_ISR_NEXT_PC() = daGetNpc(&RegSet);

    if(0 == VXDBG_ISR_NEXT_PC())
    {
    	VXDBG_ISR_NEXT_PC() = context->registers[GPR31];
    }
   ...
   /* usrBreakpointSet (VXDBG_ISR_NEXT_PC(),
			...
			VXDBG_LOCAL_FLUSH);*/
      usrBreakpointSet (VXDBG_ISR_NEXT_PC(),
    			DB_ARCH_BREAK_INST, pTcbCurrent->rtpId,
			VXDBG_LOCAL_FLUSH);
}
