...
* @brief
*       功能：
*       <li>tsmap的主体, tsMap类的实现,处理注册,查询,数据存储等</li>
*/
...
int tsMap::putpkt(US srcAid, US srcSaid, char *data, int len)
{
    ...
    /*日志处理*/
    sysLoger(LINFO, "tsMap::putpkt: recieved cmd %c", data[0]);
    sysLoger(LDEBUG, "tsMap::putpkt: recieved len:%d, cmd:%s", len, data);
    ...
    case TSMAP_QUERY:
    {
        ...
        sysLoger(LDEBUG, "tsMap::putpkt: query result:%d", iRet);
    ...
    case TSMAP_EXCEPTION:
    {
        char queryInfo[100] = {0};
        unsigned int dataLen = 0;
        char  *pStr = NULL;
        char  *pBuf = ackData;
        unsigned short desAid = 0;

        /* 设置查询信息 */
        dataLen = sprintf(queryInfo, "q:pda;0;0;%d;%s;", strlen("pda"), "pda");

        /* 查询异常上报的ID */
        iRet = query(&queryInfo[2], dataLen-2, ackData, sizeof(ackData));

        if(iRet < 0)
        {
            sysLoger(LINFO, "tsMap::query ExceptionInfo Report AID Fail");
            return FAIL;
        }
        ...
        /* 获取异常上报ID */
        pBuf += strlen("OK;");
        desAid = strtoi(pBuf, 10);

        /* 打印上报异常的ID，便于分析问题 */
        sysLoger(LINFO, "tsMap::ExceptionInfo Report AID:%d", desAid);

        /* 设置收包ID */
        curSrcAid = desAid;
        curSrcSaid = 0;

        /* 发送异常信息,接收到异常信息格式为"e:ID:%x;ExceptionInfo:..." ID为目标机ID */
        sendAck(&data[2], (int )strlen(data) - 2);
...
int tsMap::query(char * data, int len, char *ackData, int size)
{
    ...
    /*查找长度为0xFFFFFFFF时, 返回表中所有数据*/
    if(0xFFFFFFFF == args[2])
    {
        T_DATATBL::iterator m_itr = tblAddr->begin();
        int ilen = 0;

        ilen = sprintf(ackData, "OK;%x;", tblAddr->size());
        ...
        while(m_itr != tblAddr->end())
        {
            ...
            {
                ilen += sprintf(ackData + ilen, "<%x;%x;%s>",
                                (*m_itr)->regAid,
                                (*m_itr)->regSaid,
                                (*m_itr)->qryStr.c_str());
            }
