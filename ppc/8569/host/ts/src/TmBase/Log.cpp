...
void PackLog::write(PackAct act, const Packet &pack)
{
    ...
    strcat_s(info.strtime, sizeof(info.strtime), "\t");
    strcat_s(info.strtime, sizeof(info.strtime), stime);
    info.pack_act = act;
...
void PackLog::renameLogFile()
{
    ...
    //组装新文件名
    strcat_s(filenm, sizeof(filenm), "_bak");
    strcat_s(filenm, sizeof(filenm), ".log");
