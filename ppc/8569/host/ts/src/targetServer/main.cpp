...
const QString TARGET_FILENAME = "target.xml";
...
/**
* 功能：处理TClient_clearRegisterType函数,清除sa的注册类型
* @param aid sa的aid号
* @param regType 要清除的类型号,w为-1表示删除手有的对象
* @return true表示清除成功,false表示已清除了此aid
*/
...
void Manager::main()
{
    ...
    //向router注册manager对象
    char p[256];
    GetModuleFileName(NULL, p, sizeof(p));  //读入当前路径
    *(strrchr(p, '\\')) = '\0';
    appDir =  QString::fromLocal8Bit(p);
    createDirectory(const_cast<QString &>(appDir + "\\ts"));
    createDirectory(const_cast<QString &>(appDir + "\\ts\\" + QString::fromStdString(tmpfilepath)));
    runFlag = true;
    //向router 注册自身
    Router::getInstance()->registerAgent(getAID(), this);
    
    //加载TS配置文件
    ret = loadConfigure(appDir + QString("\\") + TS_FILENAME);
    if (TS_SUCCESS != ret)
    {
        return;
    }
    ...
    //加载目标机连接配置信息
    loadConfigure(appDir + QString("\\ts\\") + TARGET_FILENAME);
        ...
        else
        {
            ...
            if(pAgent->getType() == AgentBase::CLIENT_AGENT)
            {
                ...
                for(int i = 0; i < MAX_REG_TYPE; i++)
                {
                    if(agentArray[i] != NULL)
                    {
                        if(agentArray[i]->indexOf(pAgent) != -1)
                        {
...
*       TS_SUCCESS: 加载配置成功
*/
int Manager::loadConfigure(const QString name)
{
    /* 先打开基本配置信息 */
    QString tsFilename(name);
    QFile f(tsFilename);
    if (!f.open(QIODevice::ReadOnly))
    {
        sysLoger(LWARNING, "Manager::loadConfigure:Open config file:%s failure!", name.toAscii().data());
        return TS_INVALID_CFG_FILE;
    }
...
bool Manager::storeConfigure()
{
    try
    {
        QFile f(appDir + QString("\\ts\\") + TARGET_FILENAME);
