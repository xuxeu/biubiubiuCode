...
/*
 * Copyright (c) 1992-1994, 1996, 1999, 2001-2002, 2005, 2007-2009, 2012,
...
-------------------
01t,05sep13,jdw  Fix Coverity issues (WIND00433558)
01s,09jul12,zhw  Removal of warnings about LP64 (WIND00357532)
01r,02feb09,zhw  Fixed the issue simulators can not include shared memory 
                 network and MIPC.(WIND00150974)
01r,05may09,jpb  Updated logMsg() arguments for LP64 support.
01q,03sep08,ebh  Ensure that smUtilIntConnect only registers once per bus type
01p,03sep08,ebh  Add in smUtilIntMipc routine for SM and MIPC support
01o,02jul07,bwa  fixed defect 40493.
01n,18apr07,bwa  added check for smAndDshm for concurrent usage.  
01m,17mar05,bwa  added third interrupt type for TIPC bearer; merged
                 IntRoutine() and PollTask() main behaviour into one function.
...
01d,02jun92,elh  tree shuffle: changed includes to have absolute path from h/
...
#define DEFAULT_TAS_CHECKS  10      /* number of times to recheck soft tas */
#define TAS_CONST           0x80    /* added to CPU number for TAS written */
#define POLL_TASK_DELAY     0       /* in ticks                            */

/* globals */

int         smUtilVerbose     = FALSE;              /* verbose mode 	 */
int         smUtilTasChecks   = DEFAULT_TAS_CHECKS; /* rechecks for soft tas */
VOIDFUNCPTR smUtilTasClearRtn = NULL;               /* TAS clear routine     */
FUNCPTR     smUtilUser1Rtn    = NULL;               /* returns STATUS        */
FUNCPTR     smUtilUser2Rtn    = NULL;               /* returns STATUS 	 */

IMPORT int smAndDshm;	/* TRUE if both SM and DSHM are present */
IMPORT int smAndMipc;	/* TRUE if both SM and MIPC are present */

/* list of shared memory event handling routines */

SM_ROUTINE  smUtilNetRoutine =  { NULL, 0};
SM_ROUTINE  smUtilTipcRoutine = { NULL, 0};
...
#if POLL_TASK_DELAY
int         smUtilPollTaskPriority    = 50;        /* priority */
#else
...
#else
LOCAL TASK_ID smUtilPollTaskID;
#endif

 int   smUtilIntType;                         /* needed for bus int ack */
 int   smUtilIntArg1;			   /* needed for bus int ack */
/* forwards */
...
BOOL smUtilSoftTas
    ...
    unsigned int tmp; /* temp storage */

    if (smUtilTasValue == 0)
        {
        /* Combining these two lines may result in a macro parsing error */

        tmp = (TAS_CONST + sysProcNumGet()) << 24;
        smUtilTasValue = htonl(tmp);
        }
...
STATUS smUtilMemProbe
    ...
    if (smUtilVerbose)
        {
        printf ("smUtilMemProbe:op[%s] addr:[0x%p] size:[%d]\n", 
                (op == VX_READ) ? "read" : "write",  pAddr, size);
        }
...
STATUS smUtilIntGen
    (
    SM_CPU_DESC * pCpuDesc, /* local addr of destination CPU descriptor */
    int           cpuNum    /* destination cpu number */
    )
    {
    volatile long bogus;             /* bogus space */
    ...
    if ((intType >= SM_INT_MAILBOX_1) && (intType <= SM_INT_MAILBOX_R4))
        ...
        if (sysBusToLocalAdrs (intArg1, (char *) intArg2, &pMailbox) == ERROR)
            ...
            if (smUtilVerbose)
                {
                logMsg ("smUtilIntGen:unable to calculate mailbox [0x%lx]\n",
                        (_Vx_usr_arg_t) intArg2, 0, 0, 0, 0, 0);
                }
            return (ERROR);
            }
        }
...
STATUS smUtilIntConnect
    ...
    /* these are needed for bus interrupt acknowledgement */
    smUtilIntType = intType;
    smUtilIntArg1 = intArg1;
    ...
    switch (priorityType)
        ...
        case HIGH_PRIORITY:
            ...
            break;
        case TIPC_PRIORITY:
            smUtilTipcRoutine.routine = routine;
            smUtilTipcRoutine.param   = param;
            break;
        case LOW_PRIORITY:
            ...
            break;
        default:
            printf ("smUtilIntConnect: unknown interrupt priority (%d)\n",
                    priorityType);
            return (ERROR);
        }
	...
	case SM_INT_BUS:                        /* bus interrupts */
		...
		{
		registered = TRUE;
		if (smAndDshm || smAndMipc)
		    {
		    status = OK;
		    break;          /* let DSHM handle this */
		    }
		if ((intConnect (INUM_TO_IVEC ((long)intArg2), smUtilIntRoutine, 0)
		...
		{
		status = OK;		/* subsequent registration always OK */
		}
	    break;
	    }
	...
	case SM_INT_MAILBOX_R4:
		...
		if ((sysMailboxConnect ((FUNCPTR) smUtilIntRoutine, 0) != ERROR)
			&& (sysMailboxEnable ((char *)(long)intArg2) != ERROR))
		    {
		    status = OK;
		    }
		}
	...
	default:
	    ...
	    logMsg ("smUtilIntConnect:Unknown interrupt type %d\n",
		    (_Vx_usr_arg_t) intType, 0, 0, 0, 0, 0);
	    break;
	    }
	}
...
/*****************************************************************************
*
* smUtilRecvRoutine - generic shared memory receive handler
*
* This routine is the one actually calls the VxMP, TIPC and networking handlers
* when a packet or an event is received in shared memory.
*
* RETURNS: N/A
*/

_WRS_INLINE void smUtilRecvHandler (void)
    {
    ...
    if (smUtilTipcRoutine.routine != NULL)
        {
        (* smUtilTipcRoutine.routine) (smUtilTipcRoutine.param);
        }
...
/*****************************************************************************
*
* smUtilIntDshm - generic shared memory interrupt handler, called by DSHM
*
* This routine is called by the DSHM interrupt handler when both SM and DSHM
* are present in the system. If using bus interrupts, don't acknowledge them
* here: DSHM will take care of that.
*
* RETURNS: N/A
*/

void smUtilIntDshm (void)
    {
    smUtilRecvHandler ();
    }
...
/*****************************************************************************
*
* smUtilIntMipc - generic shared memory interrupt handler, called by MIPC
*
* This routine is called by the DSHM interrupt handler when both SM and MIPC
* are present in the system. If using bus interrupts, don't acknowledge them
* here: MIPC will take care of that.
*
* RETURNS: N/A
*/

void smUtilIntMipc (void)
    {
    smUtilRecvHandler ();
    }
...
* memory objects events. If the interrupt type is 'BUS', acknowledge it.
*
* RETURNS: N/A
*/
...
void smUtilIntRoutine (void)
    ...
    /* acknowledge bus interrupt for all types of SM traffic */
    
    if (smUtilIntType == SM_INT_BUS)
        {
        sysBusIntAck (smUtilIntArg1);
        }
    }
