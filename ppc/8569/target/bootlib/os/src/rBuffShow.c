...
/* Copyright 1984-2007, 2010 Wind River Systems, Inc.
 *
...
--------------------
01k,26sep10,wzc  Porting 64bit
01j,28sep10,jpb  Added memLib.h for memSysPartId.
01i,24may07,tcr  fix callback count
01h,23jan07,tcr  update show routine to track changes for SMP
01g,04apr03,to   declassified the BUFFER_DESC structure.
...
void rBuffShowInit (void)
    {
    handleShowConnect (handleTypeRbuff, (FUNCPTR)rBuffShow);
    }
...
STATUS rBuffShow
    ...
    printf ("Buffer version     : %s\n", rBuff->buffDesc.bufferTypeIdent);
    ...
    printf ("threshold (X'd)    : %lu (%u)", rBuff->info.threshold,
    ...
    printf ("\nattached callbacks:\n");
    for (n = 0; n < rBuff->numCallbacks; n++)
        {
        if (rBuff->callbackList [n] == NULL)
            break;
        printf ("    %p\n", rBuff->callbackList [n]);
        }
    ...
    if (type == 1)
        ...
        do
            {
            printf ("%03d: addr: %p buff: %p dataLen: %lu spaceAvail: %lu",
