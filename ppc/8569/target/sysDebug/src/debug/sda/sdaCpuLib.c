...
/************************外部声明*****************************/
extern void taUpdateCacheBreakList();
/************************前向声明*****************************/
...
/* 投递核间中断函数 */
static TA_IPI_EMIT_HANDLER_FUNC sdaIpiEmitFunc;

/* IPI处理函数 */
static TA_IPI_HANDLER_FUNC sdaIpiFunc;
 ...
 *    TA_OK:执行成功
 */
T_TA_ReturnCode sdaIpiEmit(TA_IPI_HANDLER_FUNC	func, UINT32 timeout)
{
...
T_TA_ReturnCode sdaIpiInit(TA_IPI_INSTALL_HANDLER_FUNC IPI_CONNECT, TA_IPI_EMIT_HANDLER_FUNC IPI_EMIT, TA_IPI_ENABLE_FUNC IPI_ENABLE)
{
	...
	/* 安装调试核间中断处理函数  */
#ifdef __PPC__
	IPI_CONNECT(TA_IPI_INTR_ID_DEBUG, (TA_IPI_HANDLER_FUNC)sdaCpuCtrlHandler, 0);
#else
	IPI_CONNECT(TA_IPI_INTR_ID_DEBUG, (TA_IPI_HANDLER_FUNC)IPI_Handle, 0);
#endif
...
T_TA_ReturnCode sdaCpuCtrlHandler(void)
{
    ...
    /* 关CPU中断 */
    TA_INT_CPU_LOCK(lockKey);

    /* 使其CPU进入空转状态 */
    sdaCpuCtrlLoop();

    /* 开中断  */
    TA_INT_CPU_UNLOCK(lockKey);

#ifdef __PPC__
    //刷新
    taUpdateCacheBreakList();
#endif
    return (TA_OK);
}
...
//    taDbgCpuCtrlEnable();
    ...
    /* 投递核间中断挂起所有CPU */
    if(TA_OK != sdaIpiEmit((TA_IPI_HANDLER_FUNC)sdaCpuCtrlHandler, TA_WAIT_FOREVER))
    {
        return (TA_IPI_EMIT_FAILED);
    }
