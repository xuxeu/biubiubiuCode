...
/* GDB读取的寄存器长度，DB_PPC_REGISTER_SIZE为异常上下文的长度，其中DAR，DSISR寄存器GDB不需要读取 */
#define DB_PPC_REGISTER_SUM_LEN ((DB_PPC_REGISTER_SIZE-2) * DB_REGISTER_UNIT_LEN)

/* 扩展后寄存器的长度 */
#define DB_PPC_REGISTER_SUM_SIZE (DB_PPC_REGISTER_SUM_LEN * DB_MEM_TO_CHAR_LEN)
/* 扩展后寄存器的长度 */
#define DB_ARCH_REGISTER_SUM_SIZE (DB_PPC_REGISTER_SUM_LEN * DB_MEM_TO_CHAR_LEN)
...
typedef enum  T_DB_RegisterName
{
    ...
    CTR = 4,
    XER = 5,
    FPSCR = 6, /* FPSCR虽然只占4字节，但是stfd指令操作的是8字节，所以需要为FPSCR保留8字节的空间*/
    DAR = 8,
    DSISR = 9,
    SPR_SUM = 6,
    ...
    CR = GPR_SUM + FGPR_SUM*2 + _CR,
    PLR = GPR_SUM + FGPR_SUM*2 + LR,
    PCTR = GPR_SUM + FGPR_SUM*2 + CTR,
    PXER = GPR_SUM + FGPR_SUM*2 + XER,
    PFPSCR = GPR_SUM + FGPR_SUM*2 + FPSCR,
    PDAR = GPR_SUM + FGPR_SUM*2 + DAR,
    PDSISR = GPR_SUM + FGPR_SUM*2 + DSISR,
} DB_RegisterName;
...
typedef struct T_DB_ExceptionContext
{
    ...
    UINT32 registers[DB_PPC_REGISTER_SIZE];
...
#define ALTIVEC_GDB_VR0_NUM (71) //vr0 - vr31 在gdb中的编号为71-102，vscr 103 vrsave 104
#define ALTIVEC_GDB_VR31_NUM (102)
#define ALTIVEC_GDB_VSCR_NUM (103)
#define ALTIVEC_GDB_VRSAVE_NUM (104)
#define ALTIVEC_CHAR_COUNT (16)
#define ALTIVEC_HEX_COUNT (32)

typedef UINT32 VEC_REG[4];              /* 128 bit vector register */

typedef struct
    {
    VEC_REG  vr[ALTIVEC_NUM_REGS];  /* VRFILE: 32 Vector Registers */
    UINT32   vscr[4];                   /* VSCR: vec status and control reg */
    UINT32   vrsave;                    /* VRSAVE: SPR 256 */
    UINT32   pad1;                      /* pad to 16 byte boundary.    */
    UINT32   pad2;
    UINT32   pad3;
    } T_DB_ExceptionVectorContext;
