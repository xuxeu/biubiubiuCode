...
/* 存放RTP ID列表 */
static int rtpIdList[MAX_RTP_NUM];
...
static T_TA_ReturnCode rseRtpHandler(T_TA_Packet *packet)
{
	...
	switch(moduleId)
	{
		...
		case RSE_TYPE_RTP_TASK_LIST:
			rseRtpTaskHandler(packet, &outSize);
			break;
 ...
 *      None
 */
void rseGetRtpId(int rtpId, int rtpIdList[])
{
 ...
 *      RTP个数
 */
int rseGetRtpIdList(int rtpIdList[])
{
	rtpIndex = 0;

	/* 获取RTP任务列表 */
	objEach(windRtpClass, (void*)rseGetRtpId, (int)rtpIdList, 0);
