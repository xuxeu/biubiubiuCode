...
void uec_write_phy_reg (eth_device *dev, int mii_id, int regnum, int value)
{
...
/* configuration has to be done through the TSEC1 MIIM regs */
int uec_read_phy_reg (eth_device *dev, int mii_id, int regnum)
{
	uec_private_t *ugeth = (uec_private_t *) dev->priv;
	uec_mii_t *ug_regs;
	enet_tbi_mii_reg_e mii_reg = (enet_tbi_mii_reg_e) regnum;
...
void marvell_phy_interface_mode (eth_device *dev,
