...
/************************外部声明*****************************/
extern void taMemProbeByteRead ();
extern void taMemProbeByteWrite ();
extern void taMemProbeShortRead ();
extern void taMemProbeShortWrite ();
extern void taMemProbeWordRead ();
extern void taMemProbeWordWrite ();
extern void taMemProbeErrorRtn ();

/************************前向声明*****************************/

#ifdef CONFIG_DELTAOS
/* 内存探测异常处理函数 */
static BOOL taMemProbeTrapHandle(int vecNum, ESFMIPS *pEsf, REG_SET *pRegSet,
		EXC_INFO *pExcInfo);
#endif
 ...
 *     FALSE:系统不继续处理异常
 */
static BOOL taMemProbeTrapHandle(int vecNum, ESFMIPS *pEsf, REG_SET *pRegSet, EXC_INFO *pExcInfo)
{
    /* 检查异常是否发生在内存探测操作 */
    if (((pRegSet->pc == (INSTR *) taMemProbeByteRead)    ||
         (pRegSet->pc == (INSTR *) taMemProbeByteWrite)   ||
         (pRegSet->pc == (INSTR *) taMemProbeShortRead)   ||
         (pRegSet->pc == (INSTR *) taMemProbeShortWrite)  ||
     	 ...
     	 (vecNum == IV_DBUS_VEC)))
    {
        ...
        pEsf->esfRegs.pc = (INSTR *)taMemProbeErrorRtn;
