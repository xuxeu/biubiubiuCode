...
typedef struct
{
    ...
    UINT32 (*attach)(TA_CTX_TYPE ctxType, UINT32 objId, UINT32 *pSid);
    UINT32 (*detach)(UINT32 sid);
    STATUS (*contextGet)(int ctxId, T_DB_ExceptionContext *context);
    STATUS (*contextSet)(int ctxId, T_DB_ExceptionContext *context);
...
{
    /* 内存操作接口  */
    UINT32	(*memWrite)(UINT32 ctxId, UINT32 dest,const UINT8 *src, UINT32 nBytes, UINT32 width);
    UINT32	(*memRead)(UINT32 ctxId, UINT32 readAddr, UINT8 *outBuf,UINT32 nBytes,UINT32 width);
    ...
    /* 内存上下文切换操作  */
    UINT32 (*memCtxSwitch)(UINT32 newCtx, int options);
