 ...
 *     Michael Sills-Lavoie(脡cole Polytechnique de Montr茅al)  - ZeroCopy support
 *******************************************************************************/
...
static size_t ibuf_free_size(InputBuf * ibuf) {
    ...
    if (ibuf->stream->cur == ibuf->buf + ibuf->buf_size) { /*如果流的当前指针指向了buf+bufsize,则将当前指针结束指针都指向buf指针*/
        ibuf->stream->cur = ibuf->stream->end = ibuf->buf;
    }
    assert(ibuf->inp >= ibuf->buf && ibuf->inp < ibuf->buf + ibuf->buf_size); /*inp的值大于buf，并且小于buf+size*/
    if (ibuf->stream->cur <= ibuf->inp) { /*当前指针小于inp指针*/
...
void ibuf_read_done(InputBuf * ibuf, size_t len) {
    ...
    ibuf->inp = inp; /*ibuf_read_done 这个函数会更新inp，将指针移动到读到这个包的长度如果超出则会将指针指向buf*/
