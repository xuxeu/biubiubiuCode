...
static T_TA_ReturnCode taVersionNumber(T_DB_Info* debugInfo,
                                    const UINT8 *inbuf,
                                    UINT32 inSize,
                                    UINT8 *outbuf,
                                    UINT32 *outSize);
...
T_TA_CommandTable taManagerCommandTable[TA_MANAGER_COMMANDNUM] =
{
    ...
    {"VN",  taVersionNumber,   TA_NEED_REPLY},/* 查询当前os版本信息 */
...

{
	...
	vm_point = versionNumber();

	/* 设置目标机体系结构为x86 */
    strcpy((char *)outbuf, vm_point);
