                              ...
                              const QXmlAttributes &  atts )
{
    ...
    if (qName.indexOf("channelInfo") == 0) 
    {
        ...
        switch(strWay.toInt()) 
        {
            ...
            case DeviceConfigBase::USB_DEVICE:  //USB
                str += strGuid + CONTROL_FLAG;
                vectorChannel.append(str);
                break;
...
void XmlHandler::createSA()
{
    ...
    pSA->setExtInfo(extInfo);
    /*Modified by tangxp for BUG NO.0000994 on 2008.4.1 [begin]*/
    /*  Modified brief: 设置通道类型*/
    pSA->setChannelType(channelType.toInt());
    /*Modified by tangxp for BUG NO.0000994 on 2008.4.1 [end]*/
       ...
       switch(deviceWay) 
        {
            ...
            case DeviceConfigBase::USB_DEVICE:
                {
                    USBConfig *pusbCfg = dynamic_cast<USBConfig*>(pConfigBase);
                    if(NULL == pusbCfg)
                    {
                        sysLoger(LWARNING, "XmlHandler::createSA: Created USB device config failure!");
                        break;
                    }
                    pusbCfg->setGUID(vec[iCount++]);
                    sysLoger(LDEBUG, "XmlHandler::createSA: Created USB device config for current channel!");
                }
                break;
