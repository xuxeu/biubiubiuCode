...
void ControlSubAgent::process(Packet* pack)
{
    try
    {
        ...
        //进行包的检查
        if(pack->getSize() < DIDLEN + (2*CONTROL_FLAG_LEN) + CONTROL_COMMAND_LEN)
        {
        ...
        switch(cmd)
        {
            ...
            case SET_TARGET_SWITCH_FLAG:
                handleSetAgentChannelSwitchState(pack);
                break;
...
void ControlSubAgent::handleCreateServerWithDll(Packet* packet)
{
    try
    {
        ...
        */
        ...
        WIN32_FIND_DATA fdData;         //查找文件所需的结构
        ...
        if(packet->getSize() < pDataStartAddr - packet->getData() + EXT_INFO_LEN + CONTROL_FLAG_LEN)
        {
            sysLoger(LWARNING, "ControlSubAgent::handleCreateServerWithDll: Received unknow format"\
                                                "packet. FILE:%s, LINE:%d", __MYFILE__, __LINE__);
            sendErrorAck(packet, ERR_UNKNOW_DATA, FAILURE_ACK_LEN);
...
void ControlSubAgent::handleCreateServer(Packet* pack)
{
    try
    {
        ...
        vec.append(buf);
        //extInfo
        if(pack->getSize() < (EXT_INFO_LEN + (pStr-pBuf) +DIDLEN+2*CONTROL_FLAG_LEN + CONTROL_COMMAND_LEN))
        {
                ...
                case USB:
                    {
                        ...
                        if(i == 0)
                        {
                            USBConfig *pusbConfig = new USBConfig();
                            if(pusbConfig != NULL)
                            {
                                pusbConfig->setDeviceType(USB);
                                pusbConfig->setGUID(strguid);
                                pSA = createSA(pusbConfig, name, isLog, way, SASTATE_INACTIVE);
                            }
                            else
                            {
                                sysLoger(LWARNING, "ControlSubAgent::handleCreateServer: Alloc the usbconfig object failure!");
                            }
                        }
                    }
                    break;
                    ...
                    if(pSA != NULL)
                    {
                        if(pSA->getState() == SASTATE_INACTIVE)
                        {
                            if(pSA->run())
                            {
                                ...
                                if(pBuf - pack->getData() < outbufsize + CONTROL_FLAG_LEN)
                                {
            ...
            case AgentBase::SERVER_AGENT:
                {
                    ...
                    {
                        /*Modified by tangxp for BUG NO.1693 on 2008年4月24日 [begin]*/
                        ...
                        /*Modified by tangxp for BUG NO.1693 on 2008年4月24日 [end]*/
                        setSuccessPacket(pack);
            ...
            case AgentBase::SERVER_AGENT:
                {
                    ...
                    if((!vec[iCount].isEmpty()) && (pServerAgent->getName() != vec[iCount]))
                    {
                        ...
                        sysLoger(LINFO, "ControlSubAgent::handleModifyServer: Modified ServerAgent name"\
                                                    " from %s to newName %s!", 
                                                    oldSAName, vec[iCount].toAscii().data());
                    }
                    //extInfo
                    iCount++;
...
void ControlSubAgent::handleRegisterServer(Packet* pack) 
{
    ...
    */
    ...
    else
    {
        ...
        //sendErrorAck(pack, getErrStrByErrValue(ret));
    }
...
* @param pack 接收的控制包
*/
void ControlSubAgent::handleChangeChannelType(Packet* pack)
{
    char *pBuf = NULL;
    char *pStr = NULL;
    int chtype = 0, iret = 0;
    US aid = 0;
    /**
     TC发送过来的控制包："DID, Vaid, channelType"
    */
    if(NULL == pack)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleChangeChannelType: Parameter pack is null pointer");
        return;
    }
    //指向AID地址
    pBuf = pack->getData() + DIDLEN + CONTROL_FLAG_LEN + CONTROL_COMMAND_LEN;
    pStr = strstr(pBuf, CONTROL_FLAG);        //是否有分割符
    if (pStr == NULL )
    {
        sysLoger(LWARNING, "ControlSubAgent::handleChangeChannelType: Unknow format packet!");
        sendErrorAck(pack, ERR_UNKNOW_DATA);
        return;
    }
    ...
    hex2int(&pBuf, &chtype);
    sysLoger(LINFO, "ControlSubAgent::handleChangeChannelType:Change server [aid = %d] current"\
                                " Channel to  %d. starting.....", aid, chtype);
    pStr += CONTROL_FLAG_LEN;
    pBuf = pStr;
    //进行通道切换
    iret = Manager::getInstance()->changeChannel(aid, chtype, pack->getSrc_aid());
    if (iret != TS_SUCCESS)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleChangeChannelType: Change server "\
                             "[aid = %d] current Channel to  %d. failure!", aid, chtype);
        sendErrorAck(pack, getErrStrByErrValue(iret));
    }
    else
    {   
        ...
        sendSuccessPacket(pack);
        sysLoger(LWARNING, "ControlSubAgent::handleChangeChannelType: Change server "\
                             "[aid = %d] current Channel to  %d. Successfully!", aid, chtype);
    //存储配置信息
    Manager::getInstance()->storeConfigure();
    }
    return;
...
/**
 * @Funcname:  handleSetAgentChannelSwitchState
 * @brief        :  设置服务代理的通道切换功能是否可用
 * @para1      : pack 接收的控制包
 * @return      : 
 * @Author     :  qingxiaohai
 *  
 * @History: 1.  Created this function on 2007年1月10日 14:05:32
 *   
**/
void ControlSubAgent::handleSetAgentChannelSwitchState(Packet* pack)
{
    
    if(NULL == pack)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState:pack is NULL"\
                                           " pionter!FILE:%s, LINE%d",  
                                        __MYFILE__, __LINE__);
        return;
    }
    /**
    TC发过来的控制消息: "DID, a aid, flag,"
    */
    char *pDest = NULL, *pStr = NULL;
    AgentBase *pAgent = NULL;
    AgentWithQueue *pAgentQ = NULL;
    ...
    if(NULL == pStr)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState:Unknow packet!"\
    ...
    {
        sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState:Unknow packet!"\
                                           " Lost CONTROL_FLAG after switch flag!");
        sendErrorAck(pack, ERR_UNKNOW_DATA);
        return;
    }
    hex2int(&pDest, &iflag);
    ...
    sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState:Starting to set SA!"\
                                       " [aid = %d] channel change flag as %d!", aid, flag);
    //查找对象
    pAgent = Manager::getInstance()->getServerAgentFromID((US)aid);
    if(NULL == pAgent)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState: didn't find the"\
    ...
    if(NULL == pAgentQ)
    {
        sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState: Dynamic cast "\
                                            " point of class Agent to AgentWithQueue failure!");
        sendErrorAck(pack, ERR_SYSTEM_ERR);
        return;
    }
    //已经被禁用该功能
    if(!pAgentQ->getChannelSwitchState())
    {
        //不是自己锁定的
        if((US)pack->getSrc_aid() != pAgentQ->getDisableChannelSwitchCAaid())
        {
            if(Manager::getInstance()->getServerAgentFromID(
                                        pAgentQ->getDisableChannelSwitchCAaid()) != NULL)
            {
                //不是自己锁定的，不能再锁定或解锁
                sysLoger(LINFO, "ControlSubAgent::handleSetAgentChannelSwitchState: Can't set the "\
                            "Channel switch state, because it was locked by other cliengAgent!");
                sendErrorAck(pack, ERR_CH_SW_FUNC_LOCKED_BY_OHTER);
                return;
            }
            //如果是其他客户端锁定的，并且在退出时忘记解锁
            //先进行系统解锁
            pAgentQ->setChannelSwitchState(true, MANAGER_AID);
        }
    }
    pAgentQ->setChannelSwitchState(flag, (US)(pack->getSrc_aid()));
        
    sendSuccessPacket(pack);
    sysLoger(LWARNING, "ControlSubAgent::handleSetAgentChannelSwitchState:Success to set SA!"\
                                       " [aid = %d] channel change flag as %d!", aid, flag);
    return;
...
void ControlSubAgent::handleDownLoadFile(Packet* pack)
{
    ...
    try
    {
        ...
        char *pDest = NULL, *pStr = NULL, *pBuf = NULL;
        string sFn;
...
void ControlSubAgent::sendErrorAck(Packet* pack, char *strErrMsg, UI size) 
{
    ...
    memcpy(pBuf, CONTROL_FLAG, CONTROL_FLAG_LEN);
    pBuf += CONTROL_FLAG_LEN;
    if(PACKET_DATASIZE - (pBuf - pack->getData()) < size)
    {
        ...
        { 
            ...
            { 
                ...
                if(i != 0)
                {
                    ...
                    strcat_s(pBuf, bufsize-strlen(pBuf), CONTROL_FLAG);                    
                }
                ...
                strcat_s(pBuf, bufsize-strlen(pBuf), achKey);                
            } 
        }
        if(retCode == ERROR_SUCCESS )
        {
