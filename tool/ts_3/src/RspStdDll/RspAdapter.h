...
private:
    ...
    /*Modified by tangxp on 2007.10.23 [begin]*/
    /*Modified brief:改为以通信协议对象收发数据*/
	T_CHAR m_ip[IP_LENGTH];     //BDI的IP地址
    T_WORD m_type;              //BDI的通信类型
    T_WORD m_timeout;           //BDI的超时时间
    T_WORD m_retry;             //BDI的重试次数
    DeviceBase *m_device;       //通信设别
    ProtocolBase *m_client;     //通信协议  
    /*Modified by tangxp on 2007.10.23 [end]*/
