...
#define DUMMY_SN_PREFIX "DUMMY-"


/*
 * @brief: 
 *      软件生成一个硬盘序列号
 * @param[in] snLen: 要生成的硬盘序列号长度
 * @param[out] pSN: 存放硬盘序列号的缓存
 * @return: 
 *      0: 成功
 *      -1: 失败
 */
int CreateDriveSerialNumber(char *pSN, int snLen)
{
    ...
    int len = snLen;
    int temp = 0;
    static char snArray[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    ...
    try
    {
        if (snLen > (strlen(DUMMY_SN_PREFIX) + 3))
        {
            strcpy(pSN, DUMMY_SN_PREFIX);
            len -= strlen(DUMMY_SN_PREFIX);
        }
        
        for(i=(snLen-len); i<len ; i++)
        {
            /* 采用随机方式生成 */
            pSN[i] = snArray[rand()%(strlen(snArray))];
        }
...
bool CheckoutHardDriveSerialNumber(char *pSN, int snLen)
{
    ...
    try
    {
        ...
        for(; i<snLen ; i++)
        {
            /* 硬盘序列号中可能存在' ','-'等字符，因此不能使用isalnum判断*/
            if((' ' != pSN[i]) && ((pSN[i] < '!') || (pSN[i] > '~')))
            {
                return false;
            }
        }
...
{
    *len = strlen(HardDriveSerialNumber);
    strcpy(id, HardDriveSerialNumber);
}
...
{
    strcpy(HardDriveSerialNumber, id);
}
