...
void taDebugRegsConvert(REG_SET *pRegSet, T_DB_ExceptionContext *context)
{
	...
	memcpy(context->registers,pRegSet->gpreg,GPR_SUM*sizeof(pRegSet->gpreg[0]));
...
void taDebugContextConvert(T_DB_ExceptionContext *context, REG_SET *pRegSet)
{
	...
	memcpy(pRegSet->gpreg,context->registers,GPR_SUM*sizeof(pRegSet->gpreg[0]));
...
#else
	cacheMipsDCFlushDebug(address, address+size, 1);
	cacheMipsICInvalidateDebug(address, address+size, 1);
#endif
}
