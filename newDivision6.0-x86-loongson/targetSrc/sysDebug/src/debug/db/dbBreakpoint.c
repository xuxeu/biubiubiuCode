...
/************************前向声明*****************************/
extern STATUS	cacheMipsOp_Hit_Writeback_Inv_D_Ls1x (char *);
extern STATUS	cacheMipsOp_Hit_Invalidate_I_Ls1x (char *);

STATUS	 cacheMipsDCFlushDebug (void *, void *, UINT32);
STATUS	 cacheMipsICInvalidateDebug (void *, void *, UINT32);
...
BOOL taDebugBpInstructCheckDebug(UINT8 *addr)
{
	UINT32 bpInstr = DB_ARCH_BREAK_INST;
	UINT8 *bpAddr = (INSTR *)&bpInstr;
    UINT32 index = 0;
...
STATUS cacheMipsDCFlushDebug
   ...
   for (addr = (char *)baseAddr; addr < (char *)endAddr; addr += step)
   {
   	cacheMipsOp_Hit_Writeback_Inv_D_Ls1x(addr);
...
STATUS cacheMipsICInvalidateDebug
    (
    void	*baseAddr,
    void	*endAddr,
    UINT	step
    ...
    for (addr = (char *)baseAddr; addr < (char *)endAddr; addr += step)
	cacheMipsOp_Hit_Invalidate_I_Ls1x(addr);
    __asm__ volatile("sync");
