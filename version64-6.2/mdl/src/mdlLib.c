...
{
	/* 调用系统的内存分配接口分配内存 */
	void * addr = loadShareTgtMemAlign(MDL_ALLOC_ALIGN_ADDRESS,MDL_ROUND_UP(size, alignment),0x800);
...
{
	/* 符号不属于该模块 */
    if (pSymbol->symRef != (ulong_t)info->moduleId)
	{
    	return (TRUE);
	}
...
UINT32 mdlGetModuleSymIdList(MODULE_ID moduleId)
{
	...
	/* 获取所有符号ID  */
	symEach(sysSymTbl, (FUNCPTR)mdlGetModuleSymId, (ulong_t)&matchInfo);
...
BOOL mdlCtxCreateDesc(const UINT8 *inbuf, MDL_CREATE_DESC *desc)
{
    ...
    switch (desc->contextType)
    {
    	case MDL_CRT_TASK:
    	    ...
    	    if (!((*(ptr++) == ':')
    	    	&& (0 != hex2int64(&ptr,&(desc->crtSpecific.task.entry)))
    	    	&& (*(ptr++) == ':' )
 ...
 */
ulong_t mdlCreateTask(MDL_CREATE_DESC *pCtxCreate)
{
	UINT32 i = 0;
	UINT32 stackSize = 0;
 ...
 */
ulong_t mdlCreateRtp(MDL_CREATE_DESC *pCtxCreate, ulong_t *textStart)
{
	RTP_ID rtpId = 0;
    UINT32 stackSize = 0;
    ...
    /* 设置代码段起始地址 */
    *textStart = (ulong_t)((WIND_RTP *)rtpId)->binaryInfo.textStart;

    return ((ulong_t)rtpId);
}
