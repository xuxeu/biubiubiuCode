 ...
 *     FALSE: 继续操作链表
 */
BOOL taTaskNodeFree(SLL_NODE *pTaskNode, ulong_t arg)
{
	/* 释放节点内存 */
 ...
 *     TRUE: 任务ID与任务节点匹配
 */
BOOL taTaskNodeMatch(SLL_NODE *pTaskNode, ulong_t tid)
{
 ...
 *     FALSE: 表示恢复链表所有任务运行
 */
BOOL taTaskNodeResume(SLL_NODE *pTaskNode, ulong_t arg)
{
	ulong_t threadId = arg;
 ...
 *     FALSE: 表示挂起链表所有任务运行
 */
BOOL taTaskNodeSuspend(SLL_NODE *pTaskNode, ulong_t arg)
{
	ulong_t threadId = arg;
 ...
 *     TRUE：需绑定任务到调试会话
 */
BOOL taTaskIsAttach(ulong_t tid, UINT32 *pSid)
{
	/* 查找创建新任务的任务是否绑定调试 */
 ...
 *     T_TASK_NODE:任务节点
 */
SLL_NODE* taTaskNodeFind(ulong_t tid, UINT32 *pSid)
{
 ...
 *     任务个数
 */
INT32 taGetTaskIdList(UINT32 sid, ulong_t idList[], INT32 maxTasks)
{
	INT32 idx = 0;
 ...
 */
T_TA_ReturnCode taAttachTask(UINT32 sid, ulong_t tid)
{
 ...
 */
T_TA_ReturnCode taDetachTask(ulong_t tid)
{
 ...
 */
UINT32 taTaskAttach(TA_CTX_TYPE ctxType, ulong_t taskId, UINT32 *pSid)
{
    UINT32 ret = TA_OK;
#ifdef _TA_CONFIG_RTP
    ulong_t objId = taRtpFromTaskGet(taskId);
#else
    ulong_t objId = 0;
#endif
...
{
	/* 获取异常任务ID */
	ulong_t threadId = TA_GET_CUR_THREAD_ID(TA_GET_SESSION_INFO(sid));
