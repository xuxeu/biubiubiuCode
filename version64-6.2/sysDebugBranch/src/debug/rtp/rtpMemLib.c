 ...
 *     oldCtx:切换前的上下文ID
 */
ulong_t taMemCtxSwitch(ulong_t newCtx)
{
    if (newCtx == 0)
    {
    	newCtx = (ulong_t)kernelId;
    }

    /* 是否支持内存切换 */
    if (TA_MEM_SWITCH_IS_SUPPORED())
    {
    	return (ulong_t)kernelId;
    }
 ...
 *     无
 */
static void taRtpFlushCache(ulong_t ctxId, ulong_t address, UINT32 size)
{
    ulong_t	savedCtx = 0;
 ...
 *      TA_FAIL: 读内存失败
 */
static UINT32 taRtpReadMemory(ulong_t ctxId, ulong_t readAddr, UINT8 *outBuf, UINT32 nBytes, T_DB_AccessMemoryAlign align)
{
    ulong_t savedCtx = 0;
    UINT32 ret = TA_OK;
    ulong_t physicalAddr = 0;
 ...
 *      TA_FAIL: 写内存失败
 */
static UINT32 taRtpWriteMemory(ulong_t ctxId, ulong_t dest, const UINT8 *src, UINT32 nBytes, T_DB_AccessMemoryAlign align)
{
    ulong_t savedCtx = 0;
    UINT32 ret = TA_OK;
    ulong_t physicalAddr = 0;
    //UINT32 pageSize = TA_RT_GET_PAGE_SIZE();获取页大小
	UINT32 pageSize = vmPageSizeGet();
    ulong_t addr = 0;
    UINT32 size = 0;
    UINT32 state = 0;
    ...
    if (pageSize != 0)
    {
	    ...
	    if ((state & VM_STATE_MASK_WRITABLE) != VM_STATE_WRITABLE)
	    {
	    	...
	    	//因为在多核调试时，该接口里面会有核间中断，需要刷新tlb表，但是多核调试状态下，核间中断被屏蔽了。
	    	//所以需要把该接口屏蔽，并在prjConfig.c文件里调用setRtpTextProtectFlag来让代码段可读写。
