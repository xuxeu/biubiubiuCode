...
/* 断点表 */
T_DB_Breakpoint dbBpTable[CONFIG_DB_BREAK_POINT];
 ...
 */
T_TA_ReturnCode taDebugLookUpBP(UINT32 sessionID, ulong_t objID, ulong_t logicAddr, T_DB_Breakpoint **breakpointList)
{
...
T_TA_ReturnCode taDebugInsertBP(T_DB_Info *debugInfo, T_DB_Breakpoint *breakpoint)
{
    ...
    /* 如果是插入软件断点 */
    if (DB_BP_NORMAL == breakpoint->bpType)
    {
        /* 插入软件断点 */
 ...
 */
T_TA_ReturnCode taDebugRemoveBP(T_DB_Info *debugInfo, ulong_t logicAddr, T_DB_BpType breakpointType, T_DB_BpClass breakpointClass, UINT32 size)
{
    ...
    while ((NULL != pMatchBP)
            ...
            || (breakpointType != pMatchBP->bpType)
    ...
    /* 如果断点类型为软件断点 */
    if (DB_BP_NORMAL == breakpointType)
    {
        /* 删除软件断点 */
 ...
 *        TA_FAIL: 操作失败
 */
T_TA_ReturnCode taDebugInsertSoftBp(ulong_t ctxId,  ulong_t addr, UINT8 *shadow)
{
 ...
 *        TA_FAIL:  操作失败
 */
T_TA_ReturnCode taDebugRemoveSoftBp(ulong_t ctxId, ulong_t addr, const UINT8 *shadow)
{
    T_TA_ReturnCode ret = TA_OK;
