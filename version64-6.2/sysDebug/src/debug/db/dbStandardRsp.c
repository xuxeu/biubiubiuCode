...
/*
 * @brief:
 *      设置64位下的线程id到传给gdb的索引的映射。
 *      目的就是将64位的线程id转化成32位的线程id，因为gdb只能识别到32位的线程id
 * @return:
 *     返回线程id对应的索引加上1(index+1)，加1的原因是索引不可为0。
 *     因为0代表当前的线程id，因此线程id到索引的映射需要偏移1.
 */
ulong_t RspThreadId64To32(ulong_t threadId)
{
	ulong_t i=0;
...
/*
 * @brief:
 *      通过传给gdb的索引的映射，来获取到64位下的线程id。
 *      目的就是将32位的线程id转化成64位的线程id，因为真实的目标机上的线程id是64位的
 * @return:
 *     返回当前64位下的线程id。
 */
ulong_t RspThreadId32To64(ulong_t id)
